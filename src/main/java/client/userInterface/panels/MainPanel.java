...
import client.userInterface.panels.tetris.singlePlayerPanels.SinglePlayerTetrioPanel;
...
public class MainPanel extends JPanel {
    ...
    public static final Font GAME_OVER_PRESS_KEY_FONT = new Font(FONT_NAME, Font.PLAIN, 20);
    ...
    private final OnlineTwoPlayersTetrioPanel onlineTetrioPanel;
    ...
    public MainPanel() {
        ...
        onlineTetrioPanel = new OnlineTwoPlayersTetrioPanel(this);
    ...
    public void connectToOnlineGame(ConnectionMode gameMode) {
        if (gameMode != ConnectionMode.JOIN_GAME) cardLayout.show(this, WAITING_OPPONENT_PANEL);
        serverConnector.connect(gameMode);
    }

    /**
     * Starts a tetris online game
     * @param seed Seed for the game
     * @param socket Socket received from the server
     * @param gameMode ConnectionMode that determines the kind of tetris game that is going to be played
     */
    public void startOnlineGame(long seed, Socket socket, ConnectionMode gameMode) {
        ...
        String onlineTwoPlayersPanelName = switch (gameMode) {
            case MODERN_TETRIS_QUICK_PLAY, HOST_GAME, JOIN_GAME -> {
                onlineTwoPlayersPanel = onlineTetrioPanel;
                yield ONLINE_TETRIO_PANEL;
            }
            case NES_QUICK_PLAY -> {
